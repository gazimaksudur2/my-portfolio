...
const Projects = () => {
	...
	return (
		<div
			id="projects"
			className="w-[90%] lg:w-[80%] mx-auto py-4 lg:py-10 flex flex-col justify-center items-start"
		>
			<ScrollRestoration />
			<h1 className="px-2 text-start bg-clip-text text-transparent bg-gradient-to-r from-purple-800 to-gray-400 text-2xl lg:text-3xl font-bold">
				Recent projects
			</h1>
			{projects && (
				<>
					<div className="mt-4 p-5 rounded-2xl bg-base-200 border-2 border-purple-400 flex flex-col-reverse lg:flex-row items-center justify-between">
						<div className="lg:w-[50%]">
							...
							<h1 className="text-[#151515bb] text-2xl font-bold">
								{projects[0]?.projectTitle}
							</h1>
							<p className="text-[#151515bb] py-4 text-sm">
							...
							<ul className="flex items-center justify-start gap-3 flex-wrap">
								{projects[0]?.skills.map((skill, idx) => (
									<>
										<li
											key={idx}
											className="bg-gradient-to-r from-[#151515db] to-[#15151580] px-3 py-1 rounded-full text-white font-semibold"
										>
											{skill}
										</li>
									</>
							...
							<div className="mt-4 text-sm flex justify-start items-center gap-4">
								<a
									...
									href={projects[0]?.liveSiteLink}
								>
									Live site
								</a>
								<a
									className="text-purple-500 hover:underline cursor-pointer"
									href={projects[0]?.githubClientRepoLink}
								>
									Github Client Repo
								</a>
								{projects[0]?.githubServerRepoLink && (
									<a
										className="text-purple-500 hover:underline cursor-pointer"
										href={projects[0]?.githubServerRepoLink}
									>
										Github Server Repo
									</a>
								)}
							...
							<div className="mt-4">
								<h4 className="text-[#151515ab] font-semibold">
									Exciting Features
								</h4>
						...
						<div className="lg:w-[45%] bg-gray-800 p-4 rounded-xl">
							<img
								className="w-[500px] max-h-[500px] object-cover"
								src={projects[0]?.bannerImgUrl}
								alt="banner image"
							/>
						</div>
					</div>
			...
			<div className="mx-auto mt-3 grid grid-cols-1 lg:grid-cols-2 gap-7">
				{projects &&
					projects.slice(1, 3).map((each, idx) => (
						<>
